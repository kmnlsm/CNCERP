<?php
/* 贷方有银行则银行交易取号多，type=1,取type=50的号/Entry of both customer receipts against accounts receivable and also general ledger or nominal receipts */

include('includes/DefineReceiptClass.php');
include('includes/session.php');


$Title = _('Receipt Entry贷方有银行则银行交易取号多，type=1,取type=50的号');
...
if (isset($_POST['CommitBatch'])){
 ...
 /* once all receipts items entered, process all the data in the
  ...
  also add the total discount total receipts*/
	...
	$result = DB_Txn_Begin();
	$_SESSION['ReceiptBatch' . $identifier]->BatchNo = GetNextTransNo(50);
	include_once('includes/Transby.php');
	addTransBy(50,$_SESSION['ReceiptBatch' . $identifier]->BatchNo);
		...
		if ($ReceiptItem->GLCode !=''){ //so its a GL receipt
			...
			/*Need to deal with the case where the payment from one bank account could be to a bank account in another currency */
				...
				*/
				...
				$PaymentTransNo = GetNextTransNo( 50 );
				include_once('includes/Transby.php');
				addTransBy(50,$PaymentTransNo);
	...
	EnsureGLEntriesBalance(50,$_SESSION['ReceiptBatch' . $identifier]->BatchNo);
