...
if (isset($_POST['submit'])  AND isset($EditName) ) { // Creating or updating a category
	...
	if (isset($SelectedCategory) and $InputError !=1 ) {
		...
		$sql = "UPDATE salescat SET salescatname = '" . $_POST['SalesCatName'] . "',
									active  = '" . $_POST['Active'] . "',
									posactive  = '" . $_POST['POSActive'] . "',
									posprinter  = '" . $_POST['POSPrinter'] . "'
...
echo '<tr>
		<td>' . _('Display in webPOS?') . ':</td>
		<td><select name="POSActive">';
if (isset ($_POST['POSActive']) && $_POST['POSActive'] == '1') {
	echo '<option selected="selected" value="1">' . _('Yes') . '</option>';
	echo '<option value="0">' . _('No') . '</option>';
} else {
	echo '<option selected="selected" value="0">' . _('No') . '</option>';
...
echo '<tr>
			<td>' . _('后厨打印机序号') . ':</td>
			<td><input type="text" name="POSPrinter" size="20" maxlength="50" value="' . $_POST['POSPrinter'] . '" /></td>
		</tr>';
...
/*$sql = "SELECT stockid,
		...
		AND mbflag<>'G'
		AND stocktype<>'M'
		ORDER BY stockid";*///原料不在POS和SHOP销售，下取消控制
...
if($result AND DB_num_rows($result)) {
	...
	echo '<table class="selection">
		...
		<tr>
			<td>' . _('Select Item') . ':</td>
			<td><select name="AddStockID"  id="AddStockID" >';
	while( $myrow = DB_fetch_array($result) ) {
	...
	//', $RootPath, '/includes/CNCERP/Getstocknamepinyin.php
	       echo '
			<script>
...
var stockid = document.getElementById("AddStockID").value;
  $.ajax({
  type: "post",
  url: "Getstocknamepinyin.php",
  data: {FormID:"' . $_SESSION['FormID'] . '" , stockid:stockid}, //提交的数据
  ...
  success: function(msg){
   ...
   $("#Codepy").val(data.pinyin);
   $("#text").html(data.pinyin ); //在#Code中输出
...
//', $_POST[Code]  ,'
	echo '</select></td>
			...
			<td onclick="id2pinyin()">' . _('POS用的助记符') . ':</td>
			<td><input type="text"  id="Codepy"  name="Code" size="20" maxlength="50" value=""  onclick="id2pinyin()"/></td>
			</tr></table>
		<br />
		<div class="centre">
